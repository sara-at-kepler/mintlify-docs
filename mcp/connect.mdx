...
<Tabs>
  <Tab title="Claude">
    ...
    <Steps>
      ...
      <Step title="Add the connector URL">
        Paste `https://mcp.kepler.ai/api/mcp` and confirm.
      </Step>
  ...
  <Tab title="Claude Code">
    ...
    ```bash
    claude mcp add --transport http kepler https://mcp.kepler.ai/api/mcp
    ```
  ...
  <Tab title="ChatGPT">
    ...
    <Steps>
      ...
      <Step title="Add the connector URL">
        Enter `https://mcp.kepler.ai/api/mcp` as the MCP server URL and confirm.
      </Step>
      <Step title="Sign in to Kepler">
  ...
  <Tab title="Cursor">
    ...
    ```json
    {
      "mcpServers": {
        "kepler": {
          "url": "https://mcp.kepler.ai/api/mcp"
        }
      }
    }
    ```
  ...
  <Tab title="Other clients">
    Any client that supports remote MCP servers over streamable HTTP can connect to Kepler. Add `https://mcp.kepler.ai/api/mcp` as a remote MCP server; Kepler signs you in with standard OAuth 2.1. No API keys to manage — authentication is handled through your Kepler login.
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="A research run seems stuck">
    Runs are meant to take a few minutes — Kepler is reading primary sources. Your assistant waits and streams progress; if a run runs long, it keeps waiting until a result or failure comes back. You can also continue the conversation or cancel the run. See [example workflows](/mcp/examples).
  </Accordion>
</AccordionGroup>
